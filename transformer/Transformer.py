...
class Head(nn.Module):
    def __init__(self, config) -> None:
        ...
        self.input_dim = config.hidden_size
        self.attention_head_size = int(config.hidden_size / config.num_attention_heads)
        self.register_parameter('weight', nn.Parameter(torch.rand((self.input_dim, self.attention_head_size * 3))))
...
class MultiHead(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.input_dim = config.hidden_size
        self.heads = nn.ModuleList([Head(config) for _ in range(self.num_attention_heads)])
        self.scale_factor = self.input_dim ** -0.5  # 1/np.sqrt(dim)
        self.softmax = nn.Softmax(dim=-1)
...
class Attention(nn.Module):
    def __init__(self, config):
        ...
        # self.self = MultiHead(config) # split multi-head
